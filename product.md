...
| Story ID | User Story | Acceptance Criteria |
|----------|------------|-------------------|
| **US2.1.1** | As an Indian user, I want the AI to automatically detect UPI transactions from SMS confirmations, so that I can track my digital payments without manual entry | • UPI SMS messages parsed with >95% accuracy<br/>• Transaction amount, merchant, and timestamp extracted correctly<br/>• Support for major UPI apps (PhonePe, Google Pay, Paytm, etc.)<br/>• Real-time processing of new SMS messages |
